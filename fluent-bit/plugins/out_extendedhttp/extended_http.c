                        ...
                        struct flb_config *config, void *data)
{
    ...
    ctx = flb_extendedhttp_conf_create(ins, config);
    if (!ctx) {
        return -1;
    }
...
flb_sds_t ext_pack_msgpack_to_json_format(
        struct flb_out_http *ctx,
        const char *data, uint64_t bytes,
        int json_format, int date_format,
        flb_sds_t date_key,
        EVP_CIPHER_CTX *cipher_ctx,
        const char *keys_to_encrypt,
        size_t amount_of_keys_to_encrypt
) {
    int i;
    int len;
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off) == ok) {
        ...
        for (i = 0; i < map_size; i++) {
            ...
            for (int ki = 0; ki < amount_of_keys_to_encrypt; ++ki) {
                if (strncmp(k->via.str.ptr, &keys_to_encrypt[ki], k->via.str.size) == 0) {
                    flb_sds_t ciphertext = flb_sds_create_len(v->via.str.ptr, v->via.str.size);
                    flb_plg_info(ctx->ins, "Encrypted Text is %s", ciphertext);
                    int outlen, rv;
                    unsigned char outbuf[1024];
                    /* Decrypt plaintext */
                    EVP_DecryptUpdate(cipher_ctx, outbuf, &outlen, ciphertext, sizeof(ciphertext));
                    /* Output decrypted block */
                    /* Finalise: note get no output for GCM */
                    rv = EVP_DecryptFinal_ex(cipher_ctx, outbuf, &outlen);
                    /* Print out return value. If this is not successful authentication
                     * failed and plaintext is not trustworthy.
                     */
                    char *plaintext = strndup((char*)outbuf, outlen);
                    flb_plg_info(ctx->ins, "Plaintext: %s - Tag Verify %s", plaintext, rv > 0 ? "Successful!" : "Failed!");

                    size_t val_size = strlen(plaintext);
                    msgpack_pack_str(&tmp_pck, val_size);
                    msgpack_pack_str_body(&tmp_pck, plaintext, val_size);
                    isEncrypted = true;
                          ...
                          struct flb_config *config)
{
    ...
    // this is only necessary if we already know the body key
    if (ctx->body_key) {
    ...
    //otherwise we use the generic output
    else if ((ctx->out_format == FLB_PACK_JSON_FORMAT_JSON) ||
        (ctx->out_format == FLB_PACK_JSON_FORMAT_STREAM) ||
        (ctx->out_format == FLB_PACK_JSON_FORMAT_LINES)) {

        char keys_to_encrypt[] = {"message"};
        json = ext_pack_msgpack_to_json_format(ctx, event_chunk->data,
                                               event_chunk->size,
                                               ctx->out_format,
...
static struct flb_config_map config_map[] = {
    ...
    {
        FLB_CONFIG_MAP_STR, "encrypt_key", NULL,
        0, FLB_TRUE, offsetof(struct flb_out_http, encrypt_key),
        "Specify the encryption key"
    },
    {
            FLB_CONFIG_MAP_STR, "encrypt_iv", NULL,
            0, FLB_TRUE, offsetof(struct flb_out_http, encrypt_iv),
            "Specify the encryption iv"
    },
...
/* Plugin reference */
struct flb_output_plugin out_extendedhttp_plugin = {
    .name        = "extendedhttp",
    .description = "Extended HTTP Output",
